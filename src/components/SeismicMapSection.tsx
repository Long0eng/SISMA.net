...
const zones: EarthquakeZone[] = [
  ...
  { id: "umbria", name: "Umbria-Marche", region: "Centro", risk: "alto", description: "Zona altamente sismica. Terremoto di Norcia nel 2016 e sequenza sismica del 1997.", recentEvents: "Magnitudo 6.5 (2016)", lat: 42.79, lng: 13.0 },
...
const riskColorMap: Record<string, { fill: string; stroke: string }> = {
  alto: { fill: "rgba(220, 50, 50, 0.5)", stroke: "#dc3232" },
  medio: { fill: "rgba(230, 160, 30, 0.5)", stroke: "#e6a01e" },
  basso: { fill: "rgba(50, 160, 220, 0.5)", stroke: "#32a0dc" },
};
...
const FlyToZone = ({ zone }: { zone: EarthquakeZone | null }) => {
  ...
  useEffect(() => {
    if (zone) {
      map.flyTo([zone.lat, zone.lng], 8, { duration: 1 });
    }
  }, [zone, map]);
  return null;
};
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <motion.div
            ...
            transition={{ duration: 0.6 }}
          >
            <div className="rounded-2xl overflow-hidden border border-border shadow-lg" style={{ height: "550px" }}>
              <MapContainer
                center={[42.0, 12.5]}
                zoom={5.5}
              ...
              >
                ...
                {zones.map((zone) => {
                  const colors = riskColorMap[zone.risk];
                  return (
                    <CircleMarker
                      key={zone.id}
            ...
            <div className="flex items-center justify-center gap-6 mt-4">
              {[
                { label: "Rischio Alto", color: "#dc3232" },
                { label: "Rischio Medio", color: "#e6a01e" },
                { label: "Rischio Basso", color: "#32a0dc" },
              ].map((item) => (
          ...
          >
            {selectedZone ? (
              ...
              <div className="rounded-2xl bg-card border border-border p-8 shadow-lg text-center">
                ...
                <p className="text-muted-foreground text-sm">Clicca su uno dei punti sulla mappa per visualizzare le informazioni sulla sismicità della zona.</p>
              </div>
            )}
              ...
              {zones.map((zone) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: riskColorMap[zone.risk].stroke }} />
                  </div>
                </button>
              ))}
            </div>
